...
private const string projectName = "LightInject.WebApi";

private const string portableClassLibraryProjectTypeGuid = "{786C830F-07A1-408B-BD7F-6EE04809D6DB}";
private const string csharpProjectTypeGuid = "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}";

string pathToSourceFile = @"..\src\LightInject.WebApi\LightInject.WebApi.cs";
...
WriteLine("LightInject.WebApi version {0}" , version);
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package/LightInject.WebApi.Source.nuspec");	
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Binary/package/LightInject.WebApi.nuspec");
	PatchNugetVersionInfo(pathToMetadataFile, version);
	NuGet.CreatePackage(pathToMetadataFile, pathToBuildDirectory);
}
...
{
	string pathToMetadata = "../src/LightInject.WebApi/NuGet";
	string pathToPackageDirectory = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package");	
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.WebApi.Source.nuspec");	
	string pathToSourceFile = "tmp/" + frameworkMoniker + "/Source/LightInject.WebApi";
	string pathToDestination = Path.Combine(pathToPackageDirectory, "content/" + packageDirectoryName + "/LightInject.WebApi");
	RoboCopy(pathToSourceFile, pathToDestination, "LightInject.WebApi.cs");
	FileUtils.Rename(Path.Combine(pathToDestination, "LightInject.WebApi.cs"), "LightInject.WebApi.cs.pp");
	ReplaceInFile(@"namespace \S*", "namespace $rootnamespace$.LightInject.WebApi", Path.Combine(pathToDestination, "LightInject.WebApi.cs.pp"));
}
...
private void CopyBinaryFile(string frameworkMoniker, string packageDirectoryName)
{
	...
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.WebApi.nuspec");
	...
	RoboCopy(pathToBinaryFile, pathToDestination, "LightInject.WebApi.*");
}

private string ResolvePathToBinaryFile(string frameworkMoniker)
{
	var pathToBinaryFile = Directory.GetFiles("tmp/" + frameworkMoniker + "/Binary/LightInject.WebApi/bin/Release","LightInject.WebApi.dll", SearchOption.AllDirectories).First();
...
{
	string pathToProjectFile = Path.Combine(pathToBuildDirectory, @"DNXCORE50/Binary/LightInject.WebApi/project.json");
	DNU.Build(pathToProjectFile, "DNXCORE50");
	
	pathToProjectFile = Path.Combine(pathToBuildDirectory, @"DNX451/Binary/LightInject.WebApi/project.json");
...
{
	string pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.WebApi");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.WebApi.Tests");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.WebApi");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.WebApi.Tests");
	NuGet.Restore(pathToProjectDirectory);
...
{
	string pathToTestAssembly = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.WebApi.Tests/bin/Release/LightInject.WebApi.Tests.dll");
...
{	
    ...
    MsTest.RunWithCodeCoverage(pathToTestAssembly, pathToPackagesFolder,pathToTestAdapterDirectory, "LightInject.WebApi.dll");
}
...
private void InitializeNugetBuildDirectory(string frameworkMoniker)
{
	...
	{
		var pathToJsonTemplateFile = Path.Combine(pathToBinary, "LightInject.WebApi/project.json_");
		var pathToJsonFile = Path.Combine(pathToBinary, "LightInject.WebApi/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
		pathToJsonTemplateFile = Path.Combine(pathToSource, "LightInject.WebApi/project.json_");
		pathToJsonFile = Path.Combine(pathToSource, "LightInject.WebApi/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
	}				  
}
...
private void Internalize(string frameworkMoniker)
{
	...
	string pathToSourceFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + "/Source/LightInject.WebApi/LightInject.WebApi.cs");
...
{
	string pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.WebApi/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
	
	pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.WebApi/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
}
...
{	
	var pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Binary\LightInject.WebApi\Properties\AssemblyInfo.cs");	
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);
	pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Source\LightInject.WebApi\Properties\AssemblyInfo.cs");
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);	
...
private void PatchInternalsVisibleToAttribute(string pathToAssemblyInfo)
{
	...
	sb.AppendLine(@"[assembly: InternalsVisibleTo(""LightInject.WebApi.Tests"")]");
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.WebApi/LightInject.WebApi.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.WebApi/LightInject.WebApi.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
...
private void SetTargetFrameworkProfile()
{
	...
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, @"PCL_111/Binary/LightInject.WebApi/LightInject.WebApi.csproj");
	XDocument xmlDocument = XDocument.Load(pathToProjectFile);	
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Binary\LightInject.WebApi.Tests\LightInject.WebApi.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);	
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Source\LightInject.WebApi.Tests\LightInject.WebApi.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
}